 ...
 *   3. 임대차계약 사실확인서
 *
 * downloadPenaltyMergedPdf — 모든 고지서를 단일 PDF로 합쳐 다운로드
 *   (3페이지 × N건). 기존 ZIP 분리 다운로드는 폐기.
 */
...
/**
 * 단일 고지서 → 주어진 doc에 3페이지 추가.
 * @param doc jsPDF instance
 * @param item 고지서
 * @param isFirstSection true면 새 doc의 첫 섹션이라 addPage 안 함
 */
function appendPenaltySection(doc: jsPDF, item: PenaltyWorkItem, isFirstSection: boolean): void {
  ...
  y += 7;
  doc.text(`발신: ${item.payer_name || '주식회사 ○○렌터카'}`, M, y);
  y += 7;
 ...
 *   파일명: 과태료_변경부과_YYYY-MM-DD_N건.pdf
 *   페이지: 3 × items.length (변경공문 + 고지서 + 사실확인서)
 */
