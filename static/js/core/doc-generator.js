...
const CONFIRMATION_STYLE = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .section-title { font-size: 14px; font-weight: bold; margin: 24px 0 8px; border-left: 3px solid #333; padding-left: 8px; }
...
/**
 * 계약사실확인서 HTML 조각 반환 (style + body)
 * — 다른 문서와 합쳐서 사용할 때
 */
export function buildConfirmationContent(data) {
  const d = { ...data, today: data.today || new Date().toISOString().split('T')[0] };
  return { style: CONFIRMATION_STYLE, body: confirmationBody(d) };
}
...
<style>${CONFIRMATION_STYLE}</style>
</head>
<body>
${confirmationBody(d)}
</body>
...
export function generateRentalConfirmation(data) {
  openPrintWindow(buildConfirmationHtml(data), '임대차계약_사실확인서');
}
