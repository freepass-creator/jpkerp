...
export async function mount() {
  gridApi = agGrid.createGrid($('#overdueGrid'), {
    columnDefs: [
      ...
      { headerName: '회차', field: 'bill_count', width: 55, type: 'numericColumn', filter: false,
        valueFormatter: p => p.value ? `${p.value}` : '' },
      { headerName: '미납액', field: 'unpaid_total', width: 110, type: 'numericColumn', filter: false,
        valueFormatter: p => fmt(p.value),
        cellStyle: { color: 'var(--c-danger)', fontWeight: 600 } },
      { headerName: '최장 연체', field: 'max_days', width: 85, type: 'numericColumn', filter: false,
        valueFormatter: p => p.value ? `${p.value}일` : '',
        cellStyle: p => ({
          color: p.value >= 30 ? '#991b1b' : p.value >= 14 ? 'var(--c-danger)' : p.value >= 7 ? 'var(--c-warn)' : 'var(--c-text-sub)',
          fontWeight: 600,
        }) },
      { headerName: '문자', field: 'sms_count', width: 55, type: 'numericColumn', filter: false,
        valueFormatter: p => p.value ? `${p.value}회` : '',
        cellStyle: { color: 'var(--c-text-sub)' } },
      { headerName: '전화', field: 'call_count', width: 55, type: 'numericColumn', filter: false,
        valueFormatter: p => p.value ? `${p.value}회` : '',
        cellStyle: { color: 'var(--c-text-sub)' } },
      { headerName: '법적조치', field: 'legal_count', width: 75, type: 'numericColumn', filter: false,
        valueFormatter: p => p.value ? `${p.value}회` : '',
