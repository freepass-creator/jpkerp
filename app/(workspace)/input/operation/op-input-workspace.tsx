...
import { type PenaltyWorkItem, usePenaltyStore } from './penalty-notice-store';
...
function OpInputWorkspaceInner() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: params만 추적 — selected 변경 시 재실행 불필요
  useEffect(() => {
    ...
    <Workspace layout="layout-254">
      ...
      <section className="panel">
        <div className="panel-head">
          <div>
            <i
              className={`ph ${opIconOf(selected).name} text-[18px]`}
              style={{ color: opIconOf(selected).color }}
            />
      ...
      <section className="panel">
        <div className="panel-head">
          <div>
            <i
              className={`ph ${isPenaltyMode ? 'ph-receipt' : isIgnitionMode ? 'ph-engine' : 'ph-clock-counter-clockwise'}`}
            />
          ...
          {isPenaltyMode && penaltyCount > 0 && (
            <div className="panel-head-actions">
              <button
                ...
                onClick={async () => {
                  try {
                    toast.info('통합 PDF 생성 중...');
                    await downloadPenaltyMergedPdf(penaltyItems, (done, total) => {
                      if (done === total) toast.success(`${total}건 통합 PDF 다운로드 완료`);
                    });
                  } catch (err) {
