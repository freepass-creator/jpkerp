...
import { INPUT_ICONS, INPUT_LABELS, INPUT_SUBS, type InputKey } from './input-types';
...
import { InsuranceCreateForm } from './forms/insurance-create-form';
import { LoanCreateForm } from './forms/loan-create-form';
...
function FormFor({ k }: { k: InputKey }) {
  switch (k) {
    ...
    case 'extension':
      return <ContractExtensionForm />;
    case 'customer':
      return <CustomerCreateForm />;
    case 'task':
      return <TaskCreateForm />;
    case 'gps':
      return <GpsCreateForm />;
    case 'partner':
      return <PartnerCreateForm />;
    case 'ocr':
      return <OcrCaptureForm />;
    case 'insurance':
      return <InsuranceCreateForm />;
    case 'loan':
  ...
  return (
    <Workspace layout="layout-254">
      ...
      <section className="panel">
        <div className="panel-head">
          <div>
            <i className={`ph ${INPUT_ICONS[selected]} text-text-sub text-[18px]`} />
            <span className="panel-title">{INPUT_LABELS[selected]}</span>
            <span className="panel-subtitle">{INPUT_SUBS[selected]}</span>
          </div>
          <div className="panel-head-actions">
            <button type="reset" form="inputForm" className="btn btn-sm btn-outline">
